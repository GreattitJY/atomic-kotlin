...
Note that the total number of cursor movements does not change
if you choose to move down and then across or across and then down.
...
The output for your program will be an integer
that is the total number of cursor movements needed to enter the string using the grid layout given.
*/
...
fun ccc08j3() {
    ...
    val string =
        input.readLine().let {
            if (it.length < 1 || it.length > 40) throw IllegalArgumentException("invalid length:  ${it.length}")
            else it
        }
    ...
    while (i < string.length) {
        if (string[i] !in "ABCDEFGHIJKLMNOPQRSTUVWXYZ -.") throw IllegalStateException("invalid string[${i}]: ${string[i]}")
        i++
    }
    ...
    for (c in string) {
        for (x in values.indices) {
            for (y in values[x].indices) {
                if (values[x][y] == c.toString()) {
                    count += abs(x - currentX) + abs(y - currentY)
...
//    마지막에 ENTER 찾기. 엔터의 위치를 상수로 가져가는 게 나을지 아니면 values에서 관리하는 게 나을지 고민...
    for (x in values.indices) {
        for (y in values[x].indices) {
            if (values[x][y] == "ENTER") {
                count += abs(x - currentX) + abs(y - currentY)
